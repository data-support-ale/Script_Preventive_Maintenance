...
class OvHandler:
    ...
    def postWLANClient(self, device_mac):
        ...
        data = {'filterConditions': [{'attribute': 'clientMac', 'value': [
            device_mac]}], 'orderBy': 'clientName', 'orderType': 'ASC', 'pageNumber': 1, 'pageSize': 1000}
        r = self.post('wma/onlineClient/getOnlineClientListByPage', data)
        python_obj = json.loads(json.dumps(
            r, sort_keys=True, indent=4, separators=(',', ': ')))
        ...
        for element in data_2:
            if "clientName" in element:
                element = json.dumps(element)
                clientName = re.findall(
                    r"\"clientName\": \"(.*?)\"", element)[0]
                print(clientName)
