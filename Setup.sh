...
count=`sudo mysql -u aletest -pwelcome@123456 aledb -e "SELECT COUNT(*) FROM ALEUser_settings_value" | grep 1`
echo $count
# COUNT(*) 2
if [ "$count" != "1" ]; then
     yz=""
     ...
     do
          ...
          case $yz in
                    [Nn]* ) 
                         sudo mysql -u aletest -pwelcome@123456 aledb -e "DELETE FROM ALEUser_settings_value where name='SETTINGS'"
                         configuration ;;
                    [Yy]* )
                         echo
                         echo "If you want to monitor specific patterns generated by OmniSwitches, please enter the list here:
...
##### WLAN Rules for WLAN Stellar AP Quality User Experience #####
...
##### WLAN Rules for WLAN Stellar AP DRM Scanning #####
...
##### WLAN Rules for WLAN Stellar AP Web Control Filtering #####
...
##### WLAN Rules for WLAN Stellar AP deassociation or deauthentication #####
...
##### WLAN Rules for WLAN Stellar AP upgrade or reboot #####
...
if \$msg contains 'pmnHALLinkStatusCallback:20' then {
     \$RepeatedMsgReduction on
     action(type=\"omfile\" DynaFile=\"deviceloghistory\" template=\"json_syslog\" DirCreateMode=\"0755\" FileCreateMode=\"0755\")
...
sudo python3 $dir/setup_called.py "$login_switch" "$pass_switch" "$mails" "$rainbow_jid" "$rainbow_jid2" "$rainbow_jid3" "$ip_server_log" "$login_AP" "$pass_AP" "$tech_pass" "$company"
