...
if jid != '':
    notif = "The SFP " + slot + "/" + port + " on OmniSwitch \"" + host + "\" IP: " + ipadd + " crossed DDM threshold " + threshold + " Power: " + sfp_power + " dBm"
...
try:
    write_api.write(bucket, org, [{"measurement": str(os.path.basename(__file__)), "tags": {"IP": ipadd, "Port" : slot + '/' + port, "Threshold" : threshold, "Power" : sfp_power}, "fields": {"count": 1}}])
...
f_logs = open('/opt/ALE_Script/{0}_ddm.txt'.format(filename),'w', errors='ignore')
f_logs.write(text)
f_logs.close()
###############################################################################
