    ...
    if link == "UPLINK":
        ...
        if save_resp == "0":
            if lldp_port_description == 0:
                ...
                info = "Preventive Maintenance Application - A port flapping has been detected on your network on the access port {0} - System Description: {1} - Number of Status Change : {2} - Link Quality : {3} on OmniSwitch {4}/{5}.\nIf you click on Yes, the following actions will be done: Port Admin Down/Up.".format(
                    port, lldp_port_description, status_changes, link_quality, ip, hostname)
                answer = send_message_request_advanced(info, jid, "Admin down")
            if answer == "2":
        ...
        if answer == '3':
            ...
            info = "Preventive Maintenance Application - A port flapping has been detected on your network and the port {0} is administratively down  on OmniSwitch {1}/{2}".format(port, ip, hostname)
            send_message(info, jid)
