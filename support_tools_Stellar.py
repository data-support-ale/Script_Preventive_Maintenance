...
def ssh_connectivity_check(ipadd,cmd):
  ...
  try:
     ...
     logging.info(runtime + ' SSH session does not establish on Stellar AP ' + ipadd)
  ...
  print(stdout)
  print(exception)
  cmd = "sshpass -p {0} ssh -v -o StrictHostKeyChecking=no {1}@{2} genrpd {3}".format(ap_password,ap_user,ipadd,"Letacla01*")
  run=cmd.split()
